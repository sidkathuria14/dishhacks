...
def main2():
    ...
    happy_buffer,sad_buffer,fear_buffer,love_buffer,angry_buffer,happy_phrases,sad_phrases,fear_phrases,love_phrases,angry_phrases,happy_location,sad_location,fear_location,love_location,angry_location,top_keywords = score_texts_emojis_v5.start(r , auth , keyword1 , 500)

    data = {'happy_buffer': happy_buffer, 'sad_buffer': sad_buffer, 'fear_buffer': fear_buffer,
            'love_buffer': love_buffer, 'angry_buffer': angry_buffer, 'total_happy': len(happy_buffer),
            'total_sad': len(sad_buffer), 'total_fear': len(fear_buffer), 'total_love': len(love_buffer),
            'total_angry': len(angry_buffer), 'happy_phrases': happy_phrases, 'sad_phrases': sad_phrases,
            'fear_phrases': fear_phrases, 'love_phrases': love_phrases, 'angry_phrases': angry_phrases,
...
def main3():
    #text_chunk = request.args.get('text_chunk')
    text_chunk = 'I installed dish a couple of months back and I am not satisfied with the service. The signal strength is' \
                 ' also weak. Please give a solution or I will switch my provider.'
    emoji_buffer = score_texts_emojis_s2t.analyse_text_chunk(text_chunk)

    #emojibuffer will be a csv file
    return emoji_buffer
